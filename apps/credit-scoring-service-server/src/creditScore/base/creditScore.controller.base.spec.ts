...
const service = {
  ...
  creditScores: () => FIND_MANY_RESULT,
  creditScore: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("CreditScore", () => {
  ...
  test("GET /creditScores/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/creditScores"}/${nonExistingId}`)
  ...
  test("GET /creditScores/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/creditScores"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /creditScores existing resource", async () => {
