/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CreditBureauWhereInput } from "./CreditBureauWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CreditBureauOrderByInput } from "./CreditBureauOrderByInput";

@ArgsType()
class CreditBureauFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CreditBureauWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CreditBureauWhereInput, { nullable: true })
  @Type(() => CreditBureauWhereInput)
  where?: CreditBureauWhereInput;

  @ApiProperty({
    required: false,
    type: [CreditBureauOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CreditBureauOrderByInput], { nullable: true })
  @Type(() => CreditBureauOrderByInput)
  orderBy?: Array<CreditBureauOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CreditBureauFindManyArgs as CreditBureauFindManyArgs };
